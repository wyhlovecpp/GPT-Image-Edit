...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    @property
    def guidance_scale_ref(self):
        return self._guidance_scale_ref
    ...
    def __call__(
        ...
        latents_ref: Optional[torch.FloatTensor] = None,
        negative_latents_ref: Optional[torch.FloatTensor] = None,
    ):
        r"""
        ...
        self._guidance_scale_ref = guidance_scale_ref
        ...
        if self.do_classifier_free_guidance:
            if self.guidance_scale_ref > 1.0:
                ...
                if latents_ref is not None and negative_latents_ref is not None:
                    latents_ref = torch.cat([negative_latents_ref, latents_ref, latents_ref], dim=0)
            else:
                ...
                if latents_ref is not None and negative_latents_ref is not None:
                    latents_ref = torch.cat([negative_latents_ref, latents_ref], dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    if self._guidance_scale_ref > 1.0:
                ...
                if latents_ref is not None:
                    latent_model_input_ref = torch.cat([latent_model_input, latents_ref], dim=-1)
                else:
                    latent_model_input_ref = latent_model_input
                ...
                if self.do_classifier_free_guidance:
                    if self._guidance_scale_ref > 1.0:
                        noise_pred_uncond, noise_pred_ref, noise_pred_ref_text = noise_pred.chunk(3)
                        noise_pred = noise_pred_uncond + \
                            self.guidance_scale_ref * (noise_pred_ref - noise_pred_uncond) + \
                            self.guidance_scale * (noise_pred_ref_text - noise_pred_ref)
                    else:
